...
//파일에서 전체 회원정보를 읽어와 ArrayList <Member> 에 저장한후 검색하거나 추가하고, 
//프로그램 종료시 파일에 회원정보 저장
public class MembersJoinManager {
	private ArrayList <Member> aMemberTable = new ArrayList<Member>();
	private String fileName;
	
	//JoinMember 생성자
	public MembersJoinManager(String file){
		fileName = file;
		memberLoadFile();
	}
	//close
	public void close() {
		memberWriteFile();		//파일에 멤버 정보 저장
	}
	//새로 가입 한 회원 정보 ArrayList 에저장 
	public void addMember(String newId, String newPwd, int iLevel, int iScore){
		if(!isMember(newId))
			aMemberTable.add(new Member(newId, newPwd, iLevel, iScore));
	}
	//ArrayList 에 멤버정보 업데이트	
	public void saveMember(String newId, String newPwd, int iLevel, int iScore){
		Member temp = new Member(newId, newPwd, iLevel, iScore);
		for(int i=0;i<aMemberTable.size();i++) {
			if(aMemberTable.get(i).getsID().equals(newId))
				aMemberTable.set(i, temp);
		}
	}
	
	//ArrayList에서 ID 검색하여 Member 리턴
...
/*********************** private ***********************************************************/
	...
	//파일에서 멤버 정보 읽어와 ArrayList 에 저장 메소드
